...
| M9 | MED | **Bulgarian infrastructure hardcoded.** `REC_HOST = https://pbx.elyoncall.com/elyon-rec.php` and the `+359` caller-ID defaults mean MK recording URLs would be signed for and streamed from the Bulgarian PBX. Contradicts "shares nothing at runtime with Bulgaria". | Latent only — `VITE_USE_REAL_VOIP=false` and telephony is Phase 2. Must be resolved before that ships. |
