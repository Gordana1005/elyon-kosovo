...
This builds the Macedonia database with the **exact same schema** as Bulgaria, the same backend
...
> must never appear in any command you run for Macedonia.
...
1. supabase.com → **New project**. Name it e.g. `elyon-macedonia`.
2. Pick a **region** close to Macedonia (EU — e.g. Frankfurt).
...
```toml
project_id = "<NEW_PROJECT_REF>"     # was sxymaloycddnoxudxaqp — DO NOT keep that
...
npx supabase link --project-ref <NEW_PROJECT_REF>      # uses the DB password
...
> Macedonia does real business. **Do not** import a Bulgarian DB dump.
...
2. Confirm the nightly recompute job exists:
   ...
   day-boundary *display* logic is changed to Macedonia time separately in
...
| Secret | Value | Notes |
|---|---|---|
| `WEBHOOK_SECRET` | a **new** random string | HMAC key for inbound webhooks; share only with Macedonia senders |
...
```bash
npx supabase secrets set WEBHOOK_SECRET=<new-random> --project-ref <NEW_PROJECT_REF>
```
...
becomes `https://<NEW_PROJECT_REF>.supabase.co/functions/v1/api/`.

> One code edit is needed before/after this: the **CORS allowlist** inside
> `supabase/functions/api/index.ts` must include the new Macedonia domain (not `elyoncall.com`).
...
✅ At this point you have an empty, secure, production-shaped Macedonia database with the backend
