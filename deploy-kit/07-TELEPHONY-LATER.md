...
(`VITE_USE_REAL_VOIP=false`). Add real calling when the business is ready and a Kosovo trunk is
...
| Item | Notes |
|---|---|
| **SIP trunk** (Kosovo/Albania carrier) | The unknown. Options to evaluate: **IPKO**, **Vodafone**, **Albtelecom**, or any SIP/VoIP wholesaler that serves +383. You need: SBC address, auth type (IP-auth vs user/pass), concurrent-channel limit, codecs. (BG uses A1 "Business Voice": IP-auth, TLS:5061, 4 channels, G.711.) |
...
| **A PBX hostname** | e.g. `pbx.elyon-xk.com`, DNS → the VPS IP. |
| **TLS cert** | Free Let's Encrypt for the PBX host (needed for WSS browser audio + trunk TLS). |

> **Cheapest shortcut:** *reuse the Sofia PBX* (point Kosovo CRM at `pbx.elyoncall.com`). It
> works, but it couples the two operations and routes through Bulgarian infrastructure/numbers —
> generally **not** recommended for a clean, independent Kosovo brand. A dedicated Kosovo PBX +
...
2. **DNS + TLS**: point `pbx.elyon-xk.com` → VPS; issue Let's Encrypt; copy cert to Asterisk
   (renewal hook), so `wss://pbx.elyon-xk.com:8089/ws` is valid.
3. **Configure the trunk** with the Kosovo carrier's details (template:
...
Once the Kosovo PBX answers:

1. Apply **Group B** edits in [06-PER-MARKET-CHANGES.md](06-PER-MARKET-CHANGES.md): replace
   every `pbx.elyoncall.com` and the Sofia DID list with the Kosovo host + +383 numbers.
