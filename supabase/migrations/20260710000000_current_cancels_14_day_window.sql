-- ============================================================================
-- ENGINE v3.2 — Current Cancels parking shortened 30d -> 14d (2026-06-23)
-- ============================================================================
-- WHY: operator decision — 30 days is too long to keep a freshly-cancelled
-- customer out of the calling rotation. Shorten the holding-pen window to 14
-- days so they re-enter their normal list (band by recency + order count, or
-- Never-Converted) sooner.
--
-- ANCHOR: the window stays measured from the cancelled order's created_at, the
-- same field every prior engine version has used. We deliberately do NOT switch
-- to orders.cancelled_at: the live data shows it is unreliable as a cancellation
-- timestamp — NULL for ~99% of recent cancels (17/1347 in the last 30 days have
-- it) and a one-time bulk backfill (1,479 rows stamped 2026-05-21) where present.
-- created_at is always populated, and recent cancels are created-and-resolved
-- the same day (median 0-day gap), so created_at is an accurate, risk-free proxy
-- for "the day they entered Current Cancels". (Verified read-only on prod DB,
-- 2026-06-23.)
--
-- This is the ONLY change vs 20260701000000_newcomers_unassigned_holding_pen.sql
-- (engine v3.1): the Current Cancels branch window 30d -> 14d. Every other rule
-- — NEWCOMERS unassigned pen, monadon_legacy exclusion, truthful freq buckets,
-- recency bands, trigger_* fields, agent-state carry-over, new-purchase reset —
-- is carried over byte-for-byte.
--
-- This file is now the CANONICAL engine (supersedes 20260701000000). Do NOT
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'prediction_segment_members_backup_20260623'
  ) THEN
    CREATE TABLE public.prediction_segment_members_backup_20260623 AS
...
BEGIN
  ...
  ELSIF v_last_cancelled_at IS NOT NULL
    ...
    -- Most recent action is a fresh cancellation: park for 14 days (was 30).
...
'engine v3.2 2026-06-23: Current Cancels parking shortened 30d -> 14d (still anchored on the cancelled order''s created_at, as before — verified that recent cancels are created+cancelled same-day, so created_at is an accurate entry-date proxy; orders.cancelled_at is unreliable/mostly NULL). All v3.1 rules retained: NEWCOMERS is an UNASSIGNED holding pen, truthful symmetric freq buckets (1-2/3-4/5-6/7+), NEWCOMERS<21d, bands 21-57/57-120/120-180/180-365/365-730/730+, never-buyer in-flight guard, monadon_legacy fully excluded, trigger_* fields written (real Last order), agent-state carry-over on band moves, new-purchase resets completed/call-again. Canonical file: 20260710000000_current_cancels_14_day_window.sql (supersedes 20260701000000). Do NOT re-run older migration SQL over this.';

-- Keep the UI card honest: the description shown on /segments comes from this column.
UPDATE public.prediction_segment_lists
SET description = 'Most recent action was a cancellation within the last 14 days. Parked here UNASSIGNED and out of every calling list; returns to the normal bucket automatically after 14 days (nightly recompute).'
WHERE name = 'Current Cancels';

COMMIT;

-- Full repair pass with the updated engine (outside the DDL transaction).
-- Immediately un-parks everyone whose cancellation is now older than 14 days,
-- so the operator does not wait for tonight's cron.
