-- Fix the frequency bucketing to be fully symmetric and always based on total paid_count.
-- 
-- Previous bug: For any client whose last paid order was ≤26, the function forced '(1-3 orders)' 
-- regardless of their actual total paid_count. This is why clients with 20 orders were in "1-3 orders" groups.
--
-- Correct rule (as user has repeated many times):
-- - last paid order price decides ≤26 vs 26+
-- - client's TOTAL paid_count decides the frequency label (1-3 / 3+ / 5+ / 7+)
-- - This must be symmetric on both price sides.
...
BEGIN
  ...
  IF v_paid_count = 0 THEN
    -- Pure non-buyer → Never-Converted
  ...
  ELSE
    IF v_last_paid_at IS NOT NULL THEN
      ...
      -- 0-21 days protection → NEWCOMERS (no price split)
      ...
      ELSE
        ...
        -- Value based ONLY on last paid price
        v_value_bucket := CASE WHEN v_last_paid_price <= 26 THEN '≤26' ELSE '26+' END;

        -- Frequency based ONLY on total paid_count (symmetric for both value sides)
        IF v_paid_count <= 3 THEN
  ...
  -- Nuclear delete for exclusivity
...
'2026-06-10 fix: Frequency bucketing now ALWAYS based on total paid_count, independent of last order price. Full symmetric matrix on both ≤26 and 26+ sides.';

-- Clean recompute so the corrected frequency logic takes effect everywhere
