-- Hotfix to make all paying clients visible in the new lifetime buckets immediately.
-- The strict "if last paid < 21 days then don't assign" was preventing almost all value clients from appearing.
-- Per current operator request: make everyone visible in the lists. The 21-day "don't spam" intent can be handled via the Cooldown Clients tool + agent notes for now.
...
BEGIN
  ...
  IF v_paid_count = 0 THEN
    -- Never-Converted logic (unchanged)
    ...
    END IF;
  ELSE
    -- Has paid history → assign to lifetime bucket (no hard 21-day block on assignment)
  ...
  -- Always clean to single winner for exclusivity
...
-- Re-populate so all clients become visible in their correct buckets
SELECT public.recompute_all_segments();

COMMIT;

COMMENT ON FUNCTION public.recompute_customer_segments(text) IS
'2026-06-06: Removed hard 21-day assignment block for paying clients so everyone is visible in the designed lifetime buckets. 21-day protection now expressed by the 21d-named buckets themselves + the Cooldown Clients UI tool.';