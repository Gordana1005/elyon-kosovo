...
import { returnsByReason } from '../db/queries/reports';
...
export const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('returns')
    .setDescription('Returns analysis by reason')
  ...
  async execute(interaction, _ctx) {
    ...
    const rows = await returnsByReason(range.startISO, range.endISO, agent.ownerId);
    if (rows.length === 0) {
      await interaction.editReply({ embeds: [infoEmbed(`No returns for ${agent.who} in ${range.label}.`)] });
      return;
    }
    ...
    const e = new EmbedBuilder()
      .setTitle(`↩️ Returns — ${agent.who}`)
      .setColor(0xec4899)
      .setDescription(clip(lines.join('\n'), 4000))
      .addFields(
        { name: 'Total returns', value: String(totalCount), inline: true },
        { name: 'Total value', value: fmtDual(totalValue), inline: true },
      )
      .setFooter({ text: `Range: ${range.label} (by returned date)` })
      .setTimestamp(new Date());

    const files = rows.length > 15 ? [csvAttachment('returns.csv', ['reason', 'count', 'value_eur'], rows.map((r) => [r.reason, r.count, r.value]))] : [];
